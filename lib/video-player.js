(function (global, factory) {
    ...
    (typeof navigator === "undefined" ? "undefined" : _typeof(navigator)) === "object" && function (global, factory) {
      ...
      var Storage = function () {
        ...
        createClass(Storage, [{
          ...
          value: function set$$1(object) {
            ...
            utils.extend(storage, object); // Update storage
      ...
      var utils = {
        ...
        loadSprite: function loadSprite(url, id) {
          ...
          function injectSprite(data) {
            ...
            this.innerHTML = data; // Inject the SVG to the body
        ...
        toggleListener: function toggleListener(elements, event, callback) {
          ...
          var capture = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : false; // Bail if no elemetns, event, or callback
        ...
        toggleState: function toggleState(element, input) {
          ...
          var state = utils.is.boolean(input) ? input : !pressed; // Set the attribute on target
        ...
        formatTime: function formatTime() {
          ...
          var secs = this.getSeconds(time); // Do we need to display hours?
        ...
        toPascalCase: function toPascalCase() {
          ...
          string = utils.replaceAll(string, '-', ' '); // Convert snake case

          string = utils.replaceAll(string, '_', ' '); // Convert to title case

          string = utils.toTitleCase(string); // Convert to pascal case
        ...
        toCamelCase: function toCamelCase() {
          ...
          string = utils.toPascalCase(string); // Convert first character to lowercase
        ...
        getUrlParams: function getUrlParams(input) {
          var search = input; // Parse URL if needed
      ...
      var html5 = {
          ...
          Object.defineProperty(player.media, 'quality', {
            ...
            set: function set(input) {
              ...
              var currentTime = player.currentTime,
                  playing = player.playing; // Set new source

              player.media.src = supported[0].getAttribute('src'); // Load new source

              player.media.load(); // Resume playing
              ...
              player.currentTime = currentTime; // Trigger change event
        ...
        cancelRequests: function cancelRequests() {
          ...
          utils.removeElement(html5.getSources()); // Set blank video src attribute
      ...
      var controls = {
        ...
        findElements: function findElements() {
          try {
            this.elements.controls = utils.getElement.call(this, this.config.selectors.controls.wrapper); // Buttons
            ...
            this.elements.progress = utils.getElement.call(this, this.config.selectors.progress); // Inputs
        ...
        createIcon: function createIcon(type, attributes) {
          ...
          var iconPath = (!iconUrl.cors ? iconUrl.url : '') + '#' + this.config.iconPrefix; // Create <svg>
        ...
        createButton: function createButton(buttonType, attr) {
          ...
          utils.setAttributes(button, attributes); // We have multiple play buttons
        ...
        createRange: function createRange(type, attributes) {
          ...
          }, i18n.get(type, this.config)); // Seek input
        ...
        createMenuItem: function createMenuItem(value, list, type, title) {
          ...
          var faux = utils.createElement('span', {
            hidden: ''
          });
        ...
        updateTimeDisplay: function updateTimeDisplay() {
          ...
          var forceHours = utils.getHours(this.duration) > 0; // eslint-disable-next-line no-param-reassign
        ...
        updateProgress: function updateProgress(event) {
          ...
          var setProgress = function setProgress(target, input) {
            ...
            var progress = utils.is.element(target) ? target : _this.elements.display.buffer; // Update value and label
          ...
          if (event) {
            switch (event.type) {
              ...
              case 'seeking':
              case 'seeked':
                value = utils.getPercentage(this.currentTime, this.duration); // Set seek range value only if it's a 'natural' time event
        ...
        updateRangeFill: function updateRangeFill(target) {
          ...
          var range = utils.is.event(target) ? target.target : target; // Needs to be a valid <input type='range'>
          ...
          range.setAttribute('aria-valuenow', range.value); // WebKit only
        ...
        updateSeekTooltip: function updateSeekTooltip(event) {
          var _this2 = this; // Bail if setting not true
        ...
        timeUpdate: function timeUpdate(event) {
          ...
          var invert = !utils.is.element(this.elements.display.duration) && this.config.invertTime; // Duration
        ...
        durationUpdate: function durationUpdate() {
          ...
          var hasDuration = utils.is.element(this.elements.display.duration); // If there's only one time display, display duration there
        ...
        setQualityMenu: function setQualityMenu(options) {
          ...
          var type = 'quality';
          var list = this.elements.settings.panes.quality.querySelector('ul'); // Set options if passed and filter based on config
          ...
          utils.emptyElement(list); // Get the badge HTML for HD, 4K etc
        ...
        updateSetting: function updateSetting(setting, container, input) {
          ...
          switch (setting) {
            ...
            default:
              value = !utils.is.empty(input) ? input : this[setting]; // Get default
        ...
        setCaptionsMenu: function setCaptionsMenu() {
          var _this4 = this; // TODO: Captions or language? Currently it's mixed


          var type = 'captions';
          var list = this.elements.settings.panes.captions.querySelector('ul'); // Toggle the pane and tab

          var toggle = captions.getTracks.call(this).length;
          controls.toggleTab.call(this, type, toggle); // Empty the menu

          utils.emptyElement(list); // Check if we need to toggle the parent
        ...
        setSpeedMenu: function setSpeedMenu(options) {
          var _this5 = this; // Do nothing if not selected
          ...
          utils.emptyElement(list); // Create items
        ...
        toggleMenu: function toggleMenu(event) {
          ...
          if (utils.is.event(event)) {
            ...
            var isButton = event.target === this.elements.buttons.settings; // If the click was inside the form or if the click
        ...
        getTabSize: function getTabSize(tab) {
          ...
          clone.removeAttribute('hidden'); // Prevent input's being unchecked due to the name being identical
        ...
        showTab: function showTab() {
          ...
          var pane = document.getElementById(target); // Nothing to show, bail
          ...
          var container = current.parentNode; // Set other toggles to be expanded false
          ...
          if (support.transitions && !support.reducedMotion) {
            ...
            var size = controls.getTabSize.call(this, pane); // Restore auto height/width
            ...
            utils.on(container, utils.transitionEndEvent, restore); // Set dimensions to target
          ...
          current.setAttribute('tabindex', -1); // Set attributes on target
          ...
          pane.removeAttribute('tabindex'); // Focus the first item
        ...
        create: function create(data) {
          var _this6 = this; // Do nothing if we want no controls
          ...
          var container = utils.createElement('div', utils.getAttributesFromSelector(this.config.selectors.controls.wrapper)); // Restart button
          ...
          if (this.config.controls.includes('progress')) {
            var progress = utils.createElement('div', utils.getAttributesFromSelector(this.config.selectors.progress)); // Seek range slider

            var seek = controls.createRange.call(this, 'seek', {
              id: 'plyr-seek-' + data.id
            });
            progress.appendChild(seek.label);
            progress.appendChild(seek.input); // Buffer progress
        ...
        inject: function inject() {
          ...
          var target = void 0; // Inject to custom location
      ...
      var captions = {
        ...
        setup: function setup() {
          ...
          utils.toggleClass(this.elements.container, this.config.classNames.captions.enabled, !utils.is.empty(captions.getTracks.call(this))); // Get tracks

          var tracks = captions.getTracks.call(this); // If no caption file exists, hide container for caption text
          ...
          var browser = utils.getBrowser(); // Fix IE captions if CORS is used
          ...
          captions.setLanguage.call(this); // Enable UI

          captions.show.call(this); // Set available languages in list
        ...
        setLanguage: function setLanguage() {
          var _this = this; // Setup HTML5 track rendering
          ...
          if (this.isHTML5 && this.isVideo) {
            ...
            var currentTrack = captions.getCurrentTrack.call(this); // Check if suported kind
        ...
        setCue: function setCue(input) {
          ...
          var currentTrack = captions.getCurrentTrack.call(this); // Only display current track
        ...
        setText: function setText(input) {
          ...
          if (utils.is.element(this.elements.captions)) {
            var content = utils.createElement('span'); // Empty the container

            utils.emptyElement(this.elements.captions); // Default to empty

            var caption = !utils.is.nullOrUndefined(input) ? input : ''; // Set the span content
        ...
        show: function show() {
          ...
          var active = this.storage.get('captions'); // Otherwise fall back to the default config
      ...
      var defaults$1 = {
        ...
        'statechange', 'qualitychange', 'qualityrequested', // Ads
      ...
      function onChange() {
        ...
        utils.dispatchEvent.call(this.player, this.target, this.active ? 'enterfullscreen' : 'exitfullscreen', true); // Trap focus in container
      ...
      function toggleFallback() {
        ...
        utils.toggleClass(this.target, this.player.config.classNames.fullscreen.fallback, toggle); // Toggle button and fire events
      ...
      var Fullscreen = function () {
        function Fullscreen(player) {
          ...
          classCallCheck(this, Fullscreen); // Keep reference to parent
      ...
      var ui = {
        ...
        build: function build() {
          ...
          ui.toggleNativeControls.call(this); // Captions

          captions.setup.call(this); // Reset volume
          ...
          utils.toggleClass(this.elements.container, this.config.classNames.pip.supported, support.pip && this.isHTML5 && this.isVideo); // Check for airplay support

          utils.toggleClass(this.elements.container, this.config.classNames.airplay.supported, support.airplay && this.isHTML5); // Add iOS class

          utils.toggleClass(this.elements.container, this.config.classNames.isIos, browser$2.isIos); // Add touch class

          utils.toggleClass(this.elements.container, this.config.classNames.isTouch, this.touch); // Ready for API calls
        ...
        setTitle: function setTitle() {
          ...
          if (utils.is.string(this.config.title) && !utils.is.empty(this.config.title)) {
            label += ', ' + this.config.title; // Set container label
        ...
        setPoster: function setPoster(poster) {
          var _this2 = this; // Set property regardless of validity


          this.media.setAttribute('poster', poster); // Bail if element is missing
        ...
        checkPlaying: function checkPlaying(event) {
          ...
          utils.toggleClass(this.elements.container, this.config.classNames.stopped, this.stopped); // Set ARIA state

          utils.toggleState(this.elements.buttons.play, this.playing); // Only update controls on non timeupdate events
        ...
        checkLoading: function checkLoading(event) {
          ...
          this.timers.loading = setTimeout(function () {
            ...
            utils.toggleClass(_this3.elements.container, _this3.config.classNames.loading, _this3.loading); // Update controls visibility
        ...
        createClass(Listeners, [{
          ...
          value: function handleKey(event) {
            ...
            if (pressed) {
              ...
              var preventDefault = [48, 49, 50, 51, 52, 53, 54, 56, 57, 32, 75, 38, 40, 77, 39, 37, 70, 67, 73, 76, 79]; // Check focused element
          ...
          value: function firstTouch() {
            this.player.touch = true; // Add touch class

            utils.toggleClass(this.player.elements.container, this.player.config.classNames.isTouch, true); // Clean up
          ...
          value: function global() {
            var toggle = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true; // Keyboard shortcuts
            ...
            utils.toggleListener(document.body, 'click', this.toggleMenu, toggle); // Detect touch by events
          ...
          value: function container() {
            var _this2 = this; // Keyboard shortcuts
            ...
            utils.on(this.player.elements.container, 'mousemove mouseleave touchstart touchmove enterfullscreen exitfullscreen', function (event) {
              var controls$$1 = _this2.player.elements.controls; // Remove button states for fullscreen
              ...
              if (show) {
                ui.toggleControls.call(_this2.player, true); // Use longer timeout for touch devices

                delay = _this2.player.touch ? 3000 : 2000;
              } // Clear timer


              clearTimeout(_this2.player.timers.controls); // Timer to prevent flicker when seeking
          ...
          value: function media() {
            var _this3 = this; // Time change on media
            ...
            if (this.player.supported.ui && this.player.config.clickToPlay && !this.player.isAudio) {
              ...
              var wrapper = utils.getElement.call(this.player, '.' + this.player.config.classNames.video); // Bail if there's no wrapper (this should never happen)
            ...
            utils.on(this.player.media, 'ratechange', function () {
              // Update UI
              controls.updateSetting.call(_this3.player, 'speed'); // Save to storage
            ...
            utils.on(this.player.media, 'languagechange', function () {
              // Update UI
              controls.updateSetting.call(_this3.player, 'captions'); // Save to storage
            ...
            utils.on(this.player.media, this.player.config.events.concat(['keyup', 'keydown']).join(' '), function (event) {
              var detail = {}; // Get error details from media
          ...
          value: function controls$$1() {
            var _this4 = this; // IE doesn't support input event, so we fallback to change


            var inputEvent = browser$3.isIE ? 'change' : 'input'; // Run default and custom handlers
            ...
            on(this.player.elements.buttons.play, 'click', this.player.togglePlay, 'play'); // Pause

            on(this.player.elements.buttons.restart, 'click', this.player.restart, 'restart'); // Rewind

            on(this.player.elements.buttons.rewind, 'click', this.player.rewind, 'rewind'); // Rewind

            on(this.player.elements.buttons.fastForward, 'click', this.player.forward, 'fastForward'); // Mute toggle

            on(this.player.elements.buttons.mute, 'click', function () {
              _this4.player.muted = !_this4.player.muted;
            }, 'mute'); // Captions toggle

            on(this.player.elements.buttons.captions, 'click', this.player.toggleCaptions); // Fullscreen toggle
            ...
            on(this.player.elements.buttons.airplay, 'click', this.player.airplay, 'airplay'); // Settings menu
            ...
            on(this.player.elements.settings.form, 'click', function (event) {
              event.stopPropagation(); // Go back to home tab on click
            ...
            on(this.player.elements.inputs.seek, 'mousedown mouseup keydown keyup touchstart touchend', function (event) {
              var seek = event.currentTarget; // Was playing before?

              var play = seek.hasAttribute('play-on-seeked'); // Done seeking
            ...
            }, 'volume'); // Polyfill for lower fill in <input type="range"> for webkit
            ...
            on(this.player.elements.controls, 'focusin focusout', function (event) {
              var _player = _this4.player,
                  ...
                  timers = _player.timers; // Skip transition to prevent focus from scrolling the parent element

              utils.toggleClass(elements.controls, config.classNames.noTransition, event.type === 'focusin'); // Toggle

              ui.toggleControls.call(_this4.player, event.type === 'focusin'); // If focusin, hide again after delay
            ...
            on(this.player.elements.inputs.volume, 'wheel', function (event) {
              ...
              var step = 1 / 50;
              var direction = 0; // Scroll down (or up on natural) to decrease
      ...
      var vimeo = {
        setup: function setup() {
          ...
          utils.toggleClass(this.elements.wrapper, this.config.classNames.embed, true); // Set intial ratio
        ...
        ready: function ready() {
          ...
          var params = utils.buildUrlParams(options); // Get the source URL or ID
          ...
          var id = utils.parseVimeoId(source); // Build an iframe
          ...
          iframe.setAttribute('allow', 'autoplay'); // Inject the package

          var wrapper = utils.createElement('div', {
            class: player.config.classNames.embedContainer
          });
          wrapper.appendChild(iframe);
          player.media = utils.replaceElement(wrapper, player.media); // Get poster image
          ...
          utils.fetch(utils.format(player.config.urls.vimeo.api, id), 'json').then(function (response) {
            ...
            url.pathname = url.pathname.split('_')[0] + '.jpg'; // Set and show poster
          ...
          Object.defineProperty(player.media, 'currentTime', {
            ...
            set: function set(time) {
              ...
              var embed = player.embed,
                  ...
                  volume = player.volume; // Set seeking state and trigger event

              media.seeking = true;
              utils.dispatchEvent.call(player, media, 'seeking'); // If paused, mute until seek is complete

              Promise.resolve(paused && embed.setVolume(0)) // Seek
              .then(function () {
          ...
          player.embed.on('progress', function (data) {
            ...
            utils.dispatchEvent.call(player, player.media, 'progress'); // Check all loaded
      ...
      var youtube = {
        setup: function setup() {
          ...
          utils.toggleClass(this.elements.wrapper, this.config.classNames.embed, true); // Set aspect ratio
        ...
        getTitle: function getTitle(videoId) {
          var _this2 = this; // Try via undocumented API method first
        ...
        ready: function ready() {
          ...
          player.media = utils.replaceElement(container, player.media); // Set poster image
            ...
            events: {
              ...
              onReady: function onReady(event) {
                ...
                Object.defineProperty(player.media, 'currentTime', {
                  ...
                  set: function set(time) {
                    ...
                    utils.dispatchEvent.call(player, player.media, 'seeking'); // Seek after events sent
                ...
                Object.defineProperty(player.media, 'quality', {
                  ...
                  set: function set(input) {
                    var quality = input; // Set via API

                    instance.setPlaybackQuality(mapQualityUnit(quality)); // Trigger request event
                ...
                utils.dispatchEvent.call(player, player.media, 'timeupdate');
                utils.dispatchEvent.call(player, player.media, 'durationchange'); // Reset timer
              ...
              onStateChange: function onStateChange(event) {
                ...
                switch (event.data) {
                  case -1:
                    // Update scrubber
                    utils.dispatchEvent.call(player, player.media, 'timeupdate'); // Get loaded % from YouTube
                  ...
                  case 1:
                    ...
                    if (player.media.paused) {
                      ...
                      assurePlaybackState$1.call(player, true);
                      utils.dispatchEvent.call(player, player.media, 'playing'); // Poll to get playback progress
      ...
      var media = {
        ...
        setup: function setup() {
          ...
          utils.toggleClass(this.elements.container, this.config.classNames.type.replace('{0}', this.type), true); // Add provider class

          utils.toggleClass(this.elements.container, this.config.classNames.provider.replace('{0}', this.provider), true); // Add video class for embeds
          ...
          if (this.isVideo) {
            ...
            utils.wrap(this.media, this.elements.wrapper); // Faux poster container
      ...
      var Ads = function () {
        ...
        createClass(Ads, [{
          ...
          value: function ready() {
            var _this3 = this; // Start ticking our safety timer. If the whole advertisement
          ...
          value: function onAdsManagerLoaded(event) {
            var _this6 = this; // Get the ads manager
          ...
          value: function listeners() {
            ...
            var container = this.player.elements.container;
            var time = void 0; // Add listeners to the required events
          ...
          value: function loadAds() {
            var _this10 = this; // Tell our adsManager to go bye bye
      ...
      var source = {
        ...
        change: function change(input) {
          ...
          this.destroy.call(this, function () {
            ...
            _this2.provider = !utils.is.empty(input.sources[0].provider) ? input.sources[0].provider : providers.html5; // Check for support

            _this2.supported = support.check(_this2.type, _this2.provider, _this2.config.playsinline); // Create new markup
      ...
      var Plyr = function () {
        function Plyr(target, options) {
          ...
          var iframe = null;
          var url = null;
          var params = null; // Different setup based on type
          ...
          switch (type) {
            case 'div':
              ...
              if (utils.is.element(iframe)) {
                ...
                this.provider = utils.getProviderByUrl(url); // Rework elements
          ...
          this.elements.container.setAttribute('tabindex', 0); // Add style hook
          ...
          this.ads = new Ads(this); // Autoplay if required
        ...
        createClass(Plyr, [{
          ...
          value: function toggleCaptions(input) {
            ...
            var show = utils.is.boolean(input) ? input : !this.elements.container.classList.contains(this.config.classNames.captions.active); // Nothing to change...
            ...
            this.captions.active = show; // Toggle state

            utils.toggleState(this.elements.buttons.captions, this.captions.active); // Add class hook

            utils.toggleClass(this.elements.container, this.config.classNames.captions.active, this.captions.active); // Trigger an event
          ...
          value: function toggleControls(toggle) {
            ...
            if (this.supported.ui && !this.isAudio) {
              ...
              var isHidden = utils.hasClass(this.elements.container, this.config.classNames.hideControls); // Negate the argument if not undefined since adding the class to hides the controls

              var force = typeof toggle === 'undefined' ? undefined : !toggle; // Apply and get updated state

              var hiding = utils.toggleClass(this.elements.container, this.config.classNames.hideControls, force); // Close menu
            ...
            var done = function done() {
              ...
              if (soft) {
                if (Object.keys(_this2.elements).length) {
                  ...
                  utils.removeElement(_this2.elements.wrapper); // Clear for GC
                ...
                // Unbind listeners
                _this2.listeners.clear(); // Replace the container with the original element provided


                utils.replaceElement(_this2.elements.original, _this2.elements.container); // Event

                utils.dispatchEvent.call(_this2, _this2.elements.original, 'destroyed', true); // Callback
          ...
          set: function set$$1(input) {
            ...
            var inputIsValid = utils.is.number(input) && input > 0; // Set
          ...
          get: function get$$1() {
            ...
            var fauxDuration = parseFloat(this.config.duration); // Media duration can be NaN before the media has loaded

            var duration = (this.media || {}).duration || 0; // If config duration is funky, use regular duration
          ...
          set: function set$$1(input) {
            ...
            this.config.quality.selected = quality; // Set quality
          ...
          set: function set$$1(input) {
            ...
            var language = input.toLowerCase(); // Check for support
            ...
            this.toggleCaptions(true); // Enabled only
            ...
            this.captions.language = language; // Clear caption

            captions.setText.call(this, null); // Update captions

            captions.setLanguage.call(this); // Trigger an event
          ...
          set: function set$$1(input) {
            ...
            var toggle = utils.is.boolean(input) ? input : this.pip === states.inline; // Toggle based on current state
  ...
  var videoPlayerClass = "js-".concat(vl.ns, "video-player"),
      dataPrefix = "data-".concat(vl.ns),
      vpAtt = "".concat(dataPrefix, "video-player"),
      vpDressedAtt = "".concat(vpAtt, "-dressed"),
      unboundVideoPlayerSelector = ".".concat(videoPlayerClass, ":not([").concat(vpDressedAtt, "]):not([data-").concat(vl.ns, "js-dress=\"false\"]), [").concat(vpAtt, "]:not([").concat(vpDressedAtt, "]):not([data-").concat(vl.ns, "js-dress=\"false\"])"),
      boundVideoPlayerSelector = ".".concat(videoPlayerClass, "[").concat(vpDressedAtt, "], [").concat(vpAtt, "][").concat(vpDressedAtt, "]");
  /**
  ...
  var VideoPlayer = /*#__PURE__*/function () {
    function VideoPlayer(playerOptions) {
      _classCallCheck(this, VideoPlayer);

      this.playerOptions = playerOptions || {};
      this.playerInstances = [];
      this.amountOfPlayersEnhanced = null;
      this.playersActive = [];
      this.players;
    }
    /**
     * Start plyr instance but only the last iteration of loop
     * @method _startPlayer
     * @param  {index} i
     */
    ...
    _createClass(VideoPlayer, [{
      key: "_startPlayer",
      value: function _startPlayer(i) {
        if (i === this.amountOfPlayersEnhanced - 1) {
          var playerInstance = new plyr(this.playersActive[i], this.playerOptions); // fix bug for audio controls not apearing in some Chrome versions

          vl.util.each(this.playersActive, function (player) {
            player.addEventListener('ready', function () {
              player.currentTime = 0.00000001;
            });
          });
      ...
      /**
       * Destroy player instance and remove class
       * @method _destroyPlayer
       * @param  {plyr-instance} playerInstance
       */

    }, {
      key: "_destroyPlayer",
      value: function _destroyPlayer(playerInstance) {
        var _this = this;

        playerInstance.destroy(function () {
          return window.setTimeout(function () {
            _this.players = document.querySelectorAll(boundVideoPlayerSelector);
            vl.util.each(_this.players, function (player) {
              player.removeAttribute(vpDressedAtt);
            });
          }, 1);
        });
      }
      ...
      key: "dress",
      value: function dress(player) {
        var i;
        player.setAttribute(vpDressedAtt, true);
        this.playersActive.push(player);
        i = this.playersActive.indexOf(player);

        this.amountOfPlayersEnhanced++;
        this._startPlayer(i);
      }
      /**
       * Selects videoPlayers and dresses them vl.util.each
       * @method dressAll
       */
      ...
      value: function dressAll() {
        ...
        this.players = document.querySelectorAll(unboundVideoPlayerSelector);
        this.amountOfPlayersEnhanced = this.players.length;
        vl.util.each(this.players, function (player) {
          _this2.dress(player);
        });
      }
    }, {
      ...
      value: function undressAll() {
        ...
        if (this.playerInstances.length) {
          vl.util.each(this.playerInstances[0], function (playerInstance) {
            if (typeof playerInstance.destroy === 'function') {
              _this3._destroyPlayer(playerInstance);
            }
          });
        }
  ...
  if (!('videoPlayer' in vl)) {
    vl.videoPlayer = new VideoPlayer({
      volume: 100
    });
